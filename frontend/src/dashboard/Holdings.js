...
const Holdings = () => {
  ...
  const pnlValues = allHoldings.map((s) => parseFloat(((s.price - s.avg) * s.qty).toFixed(2)));

  const barData = {
    labels,
    datasets: [
      { label: "Avg Cost (₹)", data: avgCosts, backgroundColor: "rgba(65, 132, 243, 0.6)", borderColor: "rgba(65, 132, 243, 1)", borderWidth: 1 },
      { label: "LTP (₹)", data: ltpValues, backgroundColor: "rgba(72, 194, 55, 0.6)", borderColor: "rgba(72, 194, 55, 1)", borderWidth: 1 },
    ],
  };
  ...
  const lineData = {
    ...
    datasets: [{
      ...
      pointBackgroundColor: pnlValues.map((v) => v >= 0 ? "rgba(72,194,55,1)" : "rgba(250,118,78,1)"),
      pointRadius: 5,
      tension: 0.4,
      fill: true,
    }],
  ...
  const chartOptions = {
    ...
    plugins: {
      legend: { position: "top" },
      title: { display: true, text: chartType === "bar" ? "Avg Cost vs LTP by Stock" : "P&L per Stock (₹)" },
  ...
  return (
    <>
      ...
      <div className="row" style={{ marginTop: "4%" }}>
        ...
        <div className="col">
          <h5 style={{ color: pnl >= 0 ? "rgb(72,194,55)" : "rgb(250,118,78)" }}>
            {pnl >= 0 ? "+" : ""}₹{pnl.toFixed(2)} ({((pnl / totalInvestment) * 100).toFixed(2)}%)
          </h5>
          <p>P&amp;L</p>
        </div>
      </div>
      ...
      <div style={{ margin: "20px 0 10px", display: "flex", gap: "10px" }}>
        <button onClick={() => setChartType("bar")} style={{ padding: "6px 16px", background: chartType === "bar" ? "#4184f3" : "#eee", color: chartType === "bar" ? "#fff" : "#333", border: "none", borderRadius: "4px", cursor: "pointer" }}>Bar Chart</button>
        <button onClick={() => setChartType("line")} style={{ padding: "6px 16px", background: chartType === "line" ? "#4184f3" : "#eee", color: chartType === "line" ? "#fff" : "#333", border: "none", borderRadius: "4px", cursor: "pointer" }}>P&amp;L Line Chart</button>
      </div>

      <div style={{ maxWidth: "100%", padding: "10px 0" }}>
        {chartType === "bar" ? <Bar options={chartOptions} data={barData} /> : <Line options={chartOptions} data={lineData} />}
      </div>
    </>
  );
};
